...
public:
    SteamApi(const asio::any_io_executor& ex, ssl::context &ctx, const char * host)
    : resolver_(ex), ex(ex), ctx(ctx), host(host){
    }
    static void fail(beast::error_code ec, char const *what) {
        std::cerr << what << ": " << ec.message() << "\n";
    }
    // Start the asynchronous operation
    void request(char const *interface, char const *method, char const *version, const std::function<void(http::response<http::string_body>)>& callback ) {
        ...
        ApiRequest apiReq(ex, ctx, host, endpoint.c_str(), callback);
        // Look up the domain name
        resolver_.async_resolve(host, "443", [&](beast::error_code ec, const net::resolver::results_type& results){apiReq.on_resolve(ec, results);});
    }

    void on_resolve(beast::error_code ec, net::resolver::results_type results) {
        if (ec)
